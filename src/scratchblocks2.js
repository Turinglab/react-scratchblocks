...
var scratchblocks2 = function () {
    ...
    block_info_by_id["_ of _"].hack = function (info, args) {
        ...
        if (func == "e^") func = "e ^";
        if (func == "10^") func = "10 ^";
        info.category = ( strings.math.contains(func) > -1) ? "operators"
                                                             : "sensing";
    }
    ...
    block_info_by_id["stop _"].hack = function (info, args) {
        ...
        info.shape = ( strings.osis.contains(what) > -1) ? null
                                                          : "cap";
    }
    ...
    function parse_block(code, context, dont_strip_brackets) {
        ...
        if (pieces.length > 1 && bracket !== "[") {
            ...
            isablock =  ["reporter", "boolean", "stack"].contains(shape) > -1;
        ...
        if (overrides) {
            for (var i=0; i<overrides.length; i++) {
                ...
                if ( override_categories.contains(value) > -1) {
                    info.category = value;
                } else if ( override_flags.contains(value) > -1) {
                    info.flag = value;
                } else if ( override_shapes.contains(value) > -1) {
                    info.shape = value;
                }
            }
    ...
    function parse_line(line, context) {
        ...
        if (comment && info.shape !== "define-hat") {
            ...
            if (match &&  override_categories.contains(match[2]) > -1) {
